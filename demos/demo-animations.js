...
(function() {
    ...
    window.addEventListener('load', () => {
        // --- Hero Title Lines
        // Note: Your HTML uses <h1>Title</h1>, not line-by-line.
        // This animation is simpler to match your existing HTML.
    ...
    gsap.utils.toArray('h2').forEach(title => {
        // Exclude footer title, as it has its own
        if (!title.closest('.footer-main')) {
            gsap.from(title, {
                opacity: 0,
                y: 50,
                duration: 1.2,
    ...
    gsap.utils.toArray('.challenge-grid, .results-grid, .impact-grid, .usecases-grid, .charts-grid, .integration-grid, .tech-grid, .pipeline-diagram, .methodology-flow').forEach(grid => {
